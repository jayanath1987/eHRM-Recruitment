...
<div class="outerbox">
    <div class="maincontent">
        ...
        <div class="mainHeading"><h2><?php echo __("Interview Summary – DG") ?></h2></div>
        <?php echo message() ?>
        ...
        <form name="standardView" id="standardView" method="post" action="<?php echo url_for('recruitment/DeleteCandidate?formId=' . 3) ?>">
            ...
            <table cellpadding="0" cellspacing="0" class="data-table">
                <thead>
                    <tr>
                        ...
                        <td scope="col">
                            <?php
                            $marks = 'c.rec_can_interview_marks';
                            ?>
                            <?php echo $sorter->sortLink($marks, __('Interview Marks'), '@DGCandidateInterview', ESC_RAW); ?>
                        </td> 
                        <td scope="col">
                            <?php
                            $status = 'c.rec_can_interview_status';
                            ?>
                            <?php echo $sorter->sortLink($status, __('Status'), '@DGCandidateInterview', ESC_RAW); ?>
                        </td> 
                        <td scope="col">
                            <?php
                            $status_hr = 'c.rec_can_interview_status_hr';
                            ?>
                            <?php echo $sorter->sortLink($status_hr, __('Admin Decision'), '@DGCandidateInterview', ESC_RAW); ?>
                        </td>   
                        <td scope="col" style="padding-left: 11px;">
                            <?php
                            $status_dg = 'c.rec_can_interview_status_dg';
                            ?>
                            <?php echo $sorter->sortLink($status_dg, __('DG Decision'), '@DGCandidateInterview', ESC_RAW); ?>
                        </td>   
                        <td scope="col">
                            <?php
                            if ($Culture == 'en') {
                            ...
                            ?>
                            <?php echo $sorter->sortLink($gender, __('Application'), '@DGCandidateInterview', ESC_RAW); ?>
                        </td>           
                        <td scope="col">
                            <?php echo '' ?>
                        </td>
                ...
                <tbody>
                    ...
                    foreach ($candidateList as $candidate) {
                        ...
                        <tr class="<?php echo $cssClass ?>">
                            ...
                            <td class="">
                                <a href="<?php echo url_for('recruitment/SaveCandidateInterview?id=' . $encrypt->encrypt($candidate->rec_can_id)."&type=DG") ?>"> <?php
                    echo $candidate->rec_can_reference_no;
                        ...
                        ?></a>            
                            ...
                            if ($Culture == 'en') {
                                if ($candidate->rec_can_interview_status == "0") {
                                    echo "Pending";
                                } else if ($candidate->rec_can_interview_status_hr == "1") {
                                    echo "Selected";
                                } else if ($candidate->rec_can_interview_status_hr == "2") {
                                    echo "Not Selected";
                                } else if ($candidate->rec_can_interview_status_hr == "3") {
                                    echo "Hold";
                                }
                            } else if ($Culture == 'si') {
                                if ($candidate->rec_can_interview_status_hr == "0") {
                                    echo "විසදීමට තිබෙන";
                                } else if ($candidate->rec_can_interview_status_hr == "1") {
                                    echo "තෝරාගත්";
                                } else if ($candidate->rec_can_interview_status_hr == "2") {
                        ...
                        ?></a>            
                            ...
                            <td class="" style="padding-bottom: 12px; padding-right: 11px;">
                                <select name="statusDG" id="parti11_<?php echo $row ?>"  class="formSelect" style="width: 60px;">
                                    ...
                                    <option value="0" <?php if ($candidate->rec_can_interview_status_dg == "0")
                                echo "selected" ?>><?php echo __("-----"); ?></option>
                                    <option value="1" <?php if ($candidate->rec_can_interview_status_dg == "1")
                                            echo "selected" ?>><?php echo __("Selected") ?></option> 
                                    <option value="2" <?php if ($candidate->rec_can_interview_status_dg == "2")
                                            echo "selected" ?>><?php echo __("Hold") ?></option>
                                </select>                                       
                            </td>
                            <td class="">
                                <a href="<?php echo url_for('recruitment/SaveCandidate?id=' . $encrypt->encrypt($candidate->rec_can_id)."&type=DG") ?>"> <?php
                                        echo __('Application');
                        ?></a>                        
                            </td>
                            <td class="">
...
//                                    }
                                    saveDG(this.id,$("#parti11_<?php echo $row ?>").val(),<?php echo $candidate->rec_can_id ?>,<?php echo $row ?>)' />
