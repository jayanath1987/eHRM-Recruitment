...
<div class="outerbox">
    <div class="maincontent">
        ...
        <div class="mainHeading"><h2><?php echo __("Request Summary - HR") ?></h2></div>
        <?php echo message() ?>
                ...
                <thead>
                    <tr>
                        ...
                        <td scope="col">
                            ...
                            ?>
                            <?php echo $sorter->sortLink($vacancy_title, __('Vacancy Title'), '@HRVacancyRequest', ESC_RAW); ?>
                        </td>
                        ...
                        <td scope="col">
                            ...
                            ?>
                            <?php echo $sorter->sortLink($vac_year, __('Year'), '@HRVacancyRequest', ESC_RAW); ?>
                        ...
                        <td scope="col">
                            ...
                            ?>
                            <?php echo $sorter->sortLink($emp_name, __('Employee Name'), '@HRVacancyRequest', ESC_RAW); ?>
                        </td>
                        <td scope="col">
                            <?php
                            $noOfVacancies = 'r.rec_vac_no_of_vacancies';
                            ?>
                            <?php echo $sorter->sortLink($noOfVacancies, __('Requested Number of Employees'), '@HRVacancyRequest', ESC_RAW); ?>
                        </td>
                        <td scope="col">
                            <?php
                            $noOfVacanciesbyHr = 'r.rec_vac_no_of_vacancies_by_hr';
                            ?>
                            <?php echo $sorter->sortLink($noOfVacanciesbyHr, __('Admin Recomended No of Employees'), '@HRVacancyRequest', ESC_RAW); ?>
                        </td>
                        <td scope="col">
                            <?php echo '' ?>
                        </td>
...
//                            die(print_r($vacancyRequestList));
                    foreach ($vacancyRequestList as $VacancyRequest) {
                        ...
                        <tr class="<?php echo $cssClass ?>">
                            ...
                            <td class="centerCol">
                                <input id="parti11_<?php echo $row ?>"  name="txtNoVacancies " type="text"  style="width: 52px; margin-left: 0px;" class="formInputText" value="<?php echo $VacancyRequest->rec_vac_no_of_vacancies_by_hr; ?>" maxlength="5" />
                            </td>
                            <td class="">
                                <input type="button" class="plainbtn editBtn" id="editBtn_<?php echo $row ?>"  value="<?php echo __("Edit") ?>" onclick='  save(this.id,$("#parti11_<?php echo $row ?>").val(),<?php echo $VacancyRequest->rec_vac_req_id ?>,<?php echo $row ?>)' />
...
function save(id,value,cId,row){
    ...
    if(mode=='edit'){
       ...
       if(fa==true){
        ...
        $.post(
        "<?php echo url_for('recruitment/UpdateHRVacancyRequest') ?>", //Ajax file
...
$(document).ready(function() {
    ...
    $("#resetBtn").click(function() {
        location.href = "<?php echo url_for(public_path('../../symfony/web/index.php/recruitment/HRVacancyRequest')) ?>";
    });

    $("#buttonRemove").click(function() {
        $("#mode").attr('value', 'delete');
        if($('input[name=chkLocID[]]').is(':checked')){
            answer = confirm("<?php echo __("Do you really want to Reject?") ?>");
        }


        else{
            alert("<?php echo __("Select at least one record to reject") ?>");
...
//                    if ($(this).val() === '' &&  $('input[name=chkLocID[]]').is(':checked')) {
